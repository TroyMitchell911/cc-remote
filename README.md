...
| **可回滚发布** | 产品版本统一为 v3.0.0，wire protocol 为 v52。构建和部署同时校验产品版本与协议版本；VPS 使用不可变 release、独立 venv、原子 `current` 切换和失败回滚，避免直接覆盖正在运行的目录。 |
...
可用状态，不会收到 `CODEX_HOME` 路径或凭据。修改后需重启 wrapper；protocol v52
...
> **升级到协议 v52**：线协议会严格拒绝版本不一致。请在同一次维护窗口部署
...
> 手工发布时先停本机 wrapper，再停服更新 relay + web，最后启动 v52 relay 和
> v52 wrapper；这样旧 wrapper 不会占住同一 `machine_id` 的连接槽。若从 v34 以前的
...
恢复，并验证旧 release 的 `/healthz`。成功后再启动 v52 wrapper。
