...
**Current release: v3.0.0** · Wire protocol v41
...
| **Rollback-safe releases** | The product version is v3.0.0 and the wire protocol is v41. Builds and deployments validate both values. The VPS uses immutable releases, release-local virtual environments, an atomic `current` switch, and rollback instead of overwriting a live directory. |
...
> **Upgrading to protocol v41:** the wire gate rejects mixed versions. Deploy
...
> then start the v41 relay and v41 wrapper so the old wrapper cannot occupy the
...
release's `/healthz` is verified. Start the v41 wrapper after success.
