...
临时会话捕获正式 ID 时迁移关联；Claude／Codex 账号 ID 变更或单／多账号切换时，
按账号身份原子迁移会话关联与撤销标记，迁移版本与记录一同保存，重启不会重复交换
账号归属。迁移失败时保留恢复标记，仅暂停受影响引擎的 Viewer 关联读写，不停用聊天。
未驻留的 Claude 历史会话从所属账号目录读取工作目录，不启动模型。
删除会话时清理其关联；fork 不复制整份列表，但其
